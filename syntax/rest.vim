...
syn region rstInlineLiteral start="``" end="``" contains=rstURL,@NoSpell
...
syn match rstDocTestBlock /^\s*\(>>>\|[.][.][.]\).*/ contains=@NoSpell skipnl nextgroup=rstDocTestBlock,rstDocTestExpectation
" XXX hack, I don't know vim's syntax at all...
syn region rstDocTestExpectation start=/^\s\+[^.> \t]/ end=/^$\|^\ze\s\+>>>/ contained contains=@NoSpell
"
...
"syn region rstQuotes start=+\"+ end=+\"+ skip=+\\"+ contains=ALLBUT,rstEmphasis,rstStrongEmphasis,rstBibliographicField,@NoSpell
" XXX: if a rstTitle title starts with a ", it starts a rstQuotes region but
" never ends it
...
if !exists("did_rst_syn_inits")
    ...
    hi link rstDocTestExpectation String
